		...
		btnAdicionarLista.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(lista.getSelectedIndex() < 0) {
					...
					JOptionPane.showMessageDialog(null, "Insira o número de capítulos lidos");
				}       		
				else{
					...
					if(assistidos > total || assistidos <= 0) {
						JOptionPane.showMessageDialog(null, "Número de capítulos maior que o total ou menor que 1, insira um número válido");
					}
					else {
						...
						if(banco.consultaListaUsuario("manga", nomeUsuario, split[0])) {
        					JOptionPane.showMessageDialog(null, "O mangá selecionado já está na sua lista");
        				}
        				else {
        					banco.adicionaListaUsuario("manga", nomeUsuario, split[0], Integer.parseInt(textCapitulosLidos.getText()), nota);		
        					JOptionPane.showMessageDialog(null, "O mangá " + split[0] + " foi adicionado á sua lista com sucesso!");
        				}    					
					}     			
				}
			}
